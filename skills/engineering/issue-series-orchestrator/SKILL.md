...
4. Process `issues.sequence` strictly in order.
5. For each issue: implementation worker → acceptance reviewer/committer → next issue.
...
3. Minimal parent check:
   - Inspect worker report.
   - Run `git status --short` and `git diff --stat`.
   - Do not perform detailed review unless needed.

4. Start acceptance reviewer/committer if review is enabled:
...
- implementation or fix worker reports that the expected implementation skill was not injected
